"""K(3,4) size-7 exhaustive search."""
...
def cycle_mask(cycle):
    mask, nn = 0, len(cycle)
    for i in range(nn): mask |= 1 << EDGE_INDEX[frozenset([cycle[i], cycle[(i+1)%nn]])]
    return mask
...
total = comb(M, 7)
print(f"N={N}, M={M}, target={TARGET}")
print(f"Size-7 search: {total:,} subsets...\n")

good = []
for i, subset in enumerate(combinations(range(M), 7)):
    if count_comps(subset) == TARGET:
        good.append(subset)
    if (i+1) % 1_000_000 == 0:
        print(f"  {i+1:,} / {total:,}, found {len(good)}")

print(f"\nFound {len(good)} size-7 generating sets")
if good:
    from collections import Counter
    print(f"\n✓ Minimum = 7 = cycle rank + 1\n")
